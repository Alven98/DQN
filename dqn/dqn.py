...
class Agent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size,
                 ...
                 mem_size=1000000, restart_training=False):
        ...
        def make_new_model_folder():
            now = datetime.now()
            n_date = str(now.date())
            n_time = str(now.timetuple()[3]) + "-" + str(now.timetuple()[4]) + "-" + str(now.timetuple()[5])
            n0 = "dqn"
            n_list = [n0, n_date, n_time]
            sep = "_"
            new_folder_name = sep.join(n_list)
            new_folder_path = os.getcwd() + '//dqn_models//' + new_folder_name
        ...
        if not restart_training:
            ...
            self.model_file = dqn_models_folder_path[latest_folder_index] + "//dqn_model.h5"
            self.load_model()
        else:
            if not os.path.isdir(os.getcwd() + '//dqn_models'):
                os.mkdir(os.getcwd() + '//dqn_models')
                path = make_new_model_folder()
